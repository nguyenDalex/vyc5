...
// jslint doesn't except function(){}() invokation; below is the work around code to wrap helper methods
commonLib = commonLib();
...
fundraisingCore = fundraisingCore();
  ...
  function getGoogleDocFeed()
  {
      ...
      script.setAttribute('src', 'https://spreadsheets.google.com/feeds/list/' + googleDocInfo.key + '/1/public/basic' + '?alt=json-in-script&callback=getFundraisingData');
...
function getFundraisingData(json) {
    var fundraisingData = {
        'donations' : 
            [],
        'total' : 0,
...
angular.module('vyc5AngularApp')
  .controller('SponsorCtrl', function ($scope) {
    ...
    fundraising.init();
    if (getFundraisingData) {
        return; // dummy line to by pass jslint 
    } 
});